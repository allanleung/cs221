...
void findStatsHelper(Node * root, double &avg, double &var, int &count) {
  ...
  int lcount, rcount;
    ...
    // Find stats in the left subtree.
  #pragma omp task shared(lavg, lvar, lcount) untied  // shared is sharing the variable across all the task
                                                      // untied allow any threads to take a new task
  { findStatsHelper(root->left, lavg, lvar, lcount); }

  // Find stats in the right subtree.
  findStatsHelper(root->right, ravg, rvar, rcount);
  #pragma omp taskwait                                // taskwait wait until all threads come back
  updateStats(root->value,
              lavg, lvar, lcount,
              ravg, rvar, rcount,
              avg, var, count);
