...
void printHeap(int* heap, int size, int node=0, int d=0) {  
  ...
  for (int i = 0; i < d; i++) { std::cout << "*"; }
    std::cout << heap[node] << endl;
    printHeap(heap, size, node * 2 + 1, d+1); // left child
    printHeap(heap, size, node * 2 + 2, d+1); // right child
}
...
int* mergeHeap(int* heap1, int* heap2, int size1, int size2) {
  ...
  for (int j = 0; j < size2; j++) {
    heap[j + size1] = heap2[j];
  }
heapify(heap, size1+size2);
return heap;
}

int test[] = {0,2,1,4,3,9,5,7,6,8};
...
int main() {
  ...
  int sizetest = sizeof(test)/sizeof(int);
  int *heaptest = new int[sizetest];
  for(int i=0; i<sizetest; ++i) {
    heaptest[i] = test[i];
  }
  heapify(heaptest, sizetest);
  cout << "test heap: ";
  printList(heaptest, sizetest);
  printHeap(heaptest, sizetest);
...
cout << "BEFORE: heap1, remove(0):";
printList(heap1, size1);
printHeap(heap1, size1);
  remove(heap1, 0, size1);
  cout << "AFTER: heap1, remove(0):";
  printList(heap1, size1);
  printHeap(heap1, size1);
...
The runtime of remove as a function of size is O(size^2) ??
The runtime of merge as a function of size is O(size1 * size 2) ????