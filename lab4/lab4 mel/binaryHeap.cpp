...
void printHeap(int *heap, int size, int node=0, int d=0) {
	...
	while (node<size) { // node is the number in the array && d is the height of the node in the tree
		if (node >> (2*d - 2) && node != 0)
			d++;
		if (d == 0 && node == 0) {
			cout << heap[d] << " " << endl;
			d++;
	...
	for (int i =0; i<size ; i++) {
		if (heap[i] == key) {
			for (int j = i; j<size ; j++) {
				heap[j] = heap[j+1]; // move all after forward
			}
			sort(heap,size);
		}
	}
...
int* mergeHeap(int* heap1, int* heap2, int size1, int size2) {
	...
	int sizeOfTwo = size1+size2;
	int newHeap[sizeOfTwo] = NULL; //initialise the new heap to NULL
    return NULL;
}
