...
//
//    if (leftchild < size) {
//        printHeap(heap, size, node = leftchild, d);
//    }
//
//    if (rightchild < size) {
//        printHeap(heap, size, node = rightchild, d);
//    }
//}


    // Part 2
    int leftchild = (2 * node) + 1; // left child number
    int rightchild = leftchild + 1; // right child is left + 1

    if (rightchild < size) { // if right is less than total size
        printHeap(heap, size, rightchild, d + 1); // run print function
    }
    // if top is false, ie right bigger than size
    // print int then add "SPACE"  * height and print node
...
void remove(int *heap, int key, int &size) {
    ...
    for(int i = 0; i < size; i++) {
        if(heap[i] == key) { // o(1) slow af
            heap[i] = heap[size-1]; // move shit left
            size--;
            heapify(heap, size);
            //swapDown(heap, i, size);
        }
...
int *mergeHeap(int *heap1, int *heap2, int size1, int size2) {
    ...
    // condition is both add + -1

    for(int i = 0, j = (size1+size2)-1; i < size1; i++,j--) {
        newHeap[j] = heap1[i];
    }
