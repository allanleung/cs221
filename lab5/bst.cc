...
int numNodes( Node* root ) {
  // tree that has nothing has height of -1 == NULL
  // tree with 1 node is 0 height
  /**
...
int numLeaves( Node* root ) {
  /**
   * THIS FUNCTION NEEDS TO BE IMPLEMENTED *
   The algorithm for numLeaves() will be very similar to numNodes(), 
   with a couple of important differences.  
   For one, the base case will be di erent.
   */
  if (root == NULL) {
  	return 0;
  }
  ...
  // RETURN ALL THE BABIES LEFT AND RIGHT
  else  {
...
void in_order( Node* rootNode, int level, Visitor& v ) {
  ...
  // can use v.visit to represent prtiing the current node.
  /**
   * THIS FUNCTION NEEDS TO BE IMPLEMENTED *
   * So basically, in-order traversal visits the nodes in ascending order:  
   * it firsts visits the left subtree, then the current node, then the right subtree.
   * An analogy for in-order traversal is arithmetic notation (3+4); operator
   * + is in-between the two operands.
